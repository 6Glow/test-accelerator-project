...
@user_router.get('/', name='All user', response_model= UserList)
def get_all_user():

      return UserList(count=len(users), users=users)


@user_router.post('/', name="Adding user", response_model=User)
def create_user(user: User):
      users.append(user)
      return user

@user_router.get('/{user_id}', name='Getting user', response_model=User)
def get_user(user_id: int):
      for i, user in enumerate(users):
            if user.id == user_id:
                  return user
      raise HTTPException(status_code=404, detil='User not found')
...
@user_router.put('/{user_id}', name='Update date user', response_model=User)
